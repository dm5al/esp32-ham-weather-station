...
# 36px for the callsign, which is the largest text on the device. ASCII only:
# a callsign needs letters and digits, and full Latin-1 plus Cyrillic at this
# size would cost far more flash than the one string justifies.
echo "==> lv_font_ui_36 (ASCII)"
$CONV --no-compress --no-prefilter --bpp 4 --size 36     --font Montserrat-Medium.ttf -r '0x20-0x7F,0xB0'     --format lvgl -o "$OUT/lv_font_ui_36.c" --force-fast-kern-format

# The 48px face renders the callsign and the large numeric readouts, so it needs
# capitals and digits but nothing else - a callsign is never lower case. Full
# coverage at this size would cost far more flash for glyphs nothing draws.
echo "==> lv_font_ui_48 (capitals and numerals)"
$CONV --no-compress --no-prefilter --bpp 4 --size 48 \
    --font Montserrat-Medium.ttf -r '0x20,0x2B,0x2D,0x2E,0x2F,0x30-0x3A,0x41-0x5A,0xB0' \
